...
- **`headless: false` + `--headless=new`** — Playwright `headless: true` использует старый headless без поддержки расширений. `headless: false` отключает авто-добавление headless-флагов, а `--headless=new` даёт новый headless с поддержкой
- **`executablePath`** — Chrome из кэша Puppeteer, не системный. Гарантирует поддержку `--load-extension`
- Manifest V3: `context.serviceWorkers()` (не `backgroundPages()`) — только если расширение имеет service worker
- **Isolated world**: content script `window.*` свойства НЕ видны из `page.evaluate()`. Детекция расширения — только через DOM
- Бэкенд стартуется Playwright через `webServer` — отдельный docker-compose не нужен
...
export const test = base.extend<{ context: BrowserContext; extensionId: string }>({
  context: async ({}, use) => {
    ...
    const context = await chromium.launchPersistentContext('', {
      headless: false,           // НЕ true! Playwright headless: true = старый headless без расширений
      executablePath: CHROME_BIN, // Puppeteer Chrome — гарантированно поддерживает --load-extension
      args: [
        '--headless=new',        // Новый headless — поддерживает расширения
...
### 2.4 Проверка подхода: test-extension-playwright/

Подход к e2e тестированию расширения проверен на минимальном тестовом расширении. Результаты: **7/7 тестов пройдено** за 8.8 секунд.

**Проект:** `test-extension-playwright/`

**Стек:**
- Playwright `@playwright/test` 1.58+
- Chrome binary из кэша Puppeteer (не системный Chrome)
- `chromium.launchPersistentContext` с `--headless=new`

**Фикстура (проверенная, рабочая):**

```typescript
// test-extension-playwright/tests/fixtures.ts
...
export const test = base.extend<{
  ...
  context: async ({}, use) => {
    const context = await chromium.launchPersistentContext('', {
      headless: false,          // НЕ использовать старый headless
...
| Факт | Детали |
|------|--------|
| `headless: false` + `--headless=new` | Playwright `headless: true` использует старый headless без поддержки расширений. Нужно `headless: false` + аргумент `--headless=new` |
| `executablePath` обязателен | Системный Chromium может не поддерживать `--load-extension`. Используем Chrome из кэша Puppeteer |
| Isolated world | Content script `window.*` свойства НЕ видны из `page.evaluate()`. Только DOM элементы общие |
| `offsetParent` и `position: fixed` | Элементы с `position: fixed` возвращают `offsetParent === null`. Для проверки видимости использовать `getComputedStyle(el).display !== 'none'` |
| Service worker не обязателен | Расширение без background/service_worker работает — фикстура `extensionId` через `serviceWorkers()` нужна только если есть service worker |

**Пройденные тесты:**
```
✓ extension content script injects marker element (777ms)
✓ extension sets window flag (via DOM proxy) (745ms)
✓ extension injects on multiple navigations (4.8s)
✓ marker element has correct styling (716ms)
✓ console log from extension is present (676ms)
✓ can detect extension via DOM query in evaluate (760ms)
✓ can read extension state via DOM evaluate (743ms)
```

**Запуск:**
```bash
cd test-extension-playwright && npx playwright test
```
...
> ⚠️ **Isolated world**: Content scripts Manifest V3 работают в изолированном мире. `window.*` свойства, установленные content script, НЕ видны из `evaluate_script`. Для детекции расширения используйте DOM-запросы (`document.getElementById`, `document.querySelector`), а не проверку `window` флагов.
...
| 2 | Deck.gl instance найден | `evaluate_script` | Fiber walk возвращает deck object (injected.js работает в main world) |
...
# E2E тесты расширения (проверка подхода)
cd test-extension-playwright && npx playwright test

# E2E тесты проекта (автоматически стартует бэкенд)
# npx playwright test  # будет доступно после реализации extension/
...
test-extension/                    # Минимальное тестовое расширение
├── manifest.json
├── content.js
└── package.json

test-extension-playwright/          # Проверка e2e подхода (7 тестов ✓)
├── package.json
├── playwright.config.ts
├── tsconfig.json
└── tests/
    ├── fixtures.ts                # Рабочая фикстура с --headless=new
    ├── extension-loads.spec.ts    # 5 тестов загрузки расширения
    └── evaluate-script.spec.ts   # 2 теста паттерна evaluate (MCP)

scripts/
├── chrome-devtools-mcp-with-extension.sh  # MCP wrapper (проверено ✓)
├── launch-chrome-with-extension.sh
└── test-mcp-extension.sh                  # MCP stdin тест (проверено ✓)
